...
class StreamToPubsub(tweepy.StreamListener):
    def __init__(self):
        super().__init__()
        self.project_id = 'egen-mod-1-project'
        self.topic_id = 'API-Pub-Sub'
...
with open('credentials.json') as file:
    tweet_credentials = json.load(file)
gcp_json_key = "egen-mod-1-project.json"
logging.basicConfig(level = logging.INFO)

if __name__ == "__main__":
    stream_to_pubsub = StreamToPubsub()
    stream_to_pubsub.twitter_api_connect(twitter_credentials=tweet_credentials)
    ...
    stream.filter(track=["heat"], languages=['en'])
